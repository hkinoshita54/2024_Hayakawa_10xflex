...
table(seu$celltype)
seu <- subset(seu, idents = "Epi-PT", invert = TRUE)

# infercnv ----
cts <- seu[["RNA"]]$counts
anno <- data.frame(cell_name = colnames(seu), celltype = seu$celltype)
...
infercnv_obj = CreateInfercnvObject(raw_counts_matrix = cts,
                                    ...
                                    ref_group_names = levels(seu$celltype)[1:8])
infercnv_obj = infercnv::run(infercnv_obj,
                             cutoff = 0.1,  # use 1 for smart-seq, 0.1 for 10x-genomics
                             out_dir = file.path("result", "100_infercnv", "output_dir_WT_PTC_only"),  # dir is auto-created for storing outputs
                             cluster_by_groups = T,   # cluster
                             denoise = T,
                             HMM = T)
seu = infercnv::add_to_seurat(infercnv_output_path = file.path("result", "100_infercnv", "output_dir_WT_PTC_only"),
