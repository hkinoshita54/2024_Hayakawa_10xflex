...
analysis_step <- "041.1_clustering_endothelial_2"
...
seu <- subset(seu, subset = celltype %in% c("Endo."))

# Clustering ----
seu <- recluster(seu, npcs = 50, res = 1)
DimPlot(seu, label = TRUE, repel = TRUE, cols = "polychrome") + NoAxes()
markers <- FindAllMarkers(seu, only.pos = TRUE) # Check markers > c8 immune, c9 epithelial ?? 

seu <- subset(seu, idents = c(8,9), invert = TRUE)
seu <- recluster(seu, npcs = 50, res = 1)
DimPlot(seu, label = TRUE, repel = TRUE, cols = "polychrome") + NoAxes()
...
add_feat <- c("Eng", "Cd38", "Mctp1", "Kit", "Mki67")
...
seu$celltype_2[seu$seurat_clusters %in% c(0)] <- "CapEC"
seu$celltype_2[seu$seurat_clusters %in% c(1)] <- "TumEC1"
seu$celltype_2[seu$seurat_clusters %in% c(2)] <- "TumEC2"
seu$celltype_2[seu$seurat_clusters %in% c(6)] <- "TumEC3"
seu$celltype_2[seu$seurat_clusters %in% c(7)] <- "LEC"
seu$celltype_2[seu$seurat_clusters %in% c(3)] <- "TumLEC"
seu$celltype_2 <- factor(seu$celltype_2, levels = c("ArtEC", "VenEC", "CapEC", "TumEC1", "TumEC2", "TumEC3", "LEC", "TumLEC"))
...
# dotplot
features = c("Sema3g", "Ackr1", "Rgcc", "Mctp1", "Kit", "Top2a", "Prox1")
...
dotplot(kegg) +   theme(axis.text.y = element_text(size = 8, lineheight = 0.6),
                        axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
ggsave("kegg.png", path = plot_path, width = 5, height = 8, units = "in", dpi = 150)
...
dotplot(reactome) +   theme(axis.text.y = element_text(size = 8, lineheight = 0.6),
                            axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
ggsave("reactome.png", path = plot_path, width = 5, height = 6, units = "in", dpi = 150)
...
dotplot(wp) +   theme(axis.text.y = element_text(size = 8, lineheight = 0.6),
                      axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
ggsave("wp.png", path = plot_path, width = 5, height = 6, units = "in", dpi = 150)
...
# ssGSEA ----
seu_all <- seu
seu <- subset(seu, subset = celltype_2 %in% c("ArtEC", "VenEC", "CapEC", "TumEC1", "TumEC2", "TumEC3"))
...
seu$celltype_2 <- factor(seu$celltype_2, levels = c(levels(seu$celltype)[-c(3)], levels(seu2$celltype_2)))
Idents(seu) <- "celltype_2"
