...
analysis_step <- "060_reference_spatial_PTC"
...
seu <- subset(seu_all, subset = orig.ident %in% c("PTC") | celltype1 %in% c("Squam.", "Bcell", "Glial", "Neural"))
seu <- subset(seu, subset = celltype1 %in% c("Stem", "Pit", "EEC", "Tuft", "Adipo.", "Meso."), invert = TRUE)
...
seu$celltype4 <- factor(
  seu$celltype4, 
  levels = c("Tumor", "Squam.", "Bcell", "Tcell", "MNP", "TAN", "EC", "Fibro.", "Myo.", "ENS")
)
...
saveRDS(seu_ref, "RDSfiles/seu_060_ref_PTC.RDS")
...
saveRDS(seu_rctd, "RDSfiles/seu_060_ref_PTC_rctd.RDS")
