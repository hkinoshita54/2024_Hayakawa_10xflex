# Continued from 040.1_clustering_str_2.R
# Without integration
analysis_step <- "042.1_clustering_fibroblasts"
...
seu <- subset(seu, subset = celltype %in% c("Fib."))

# Clustering ----
seu <- recluster(seu, npcs = 50, res = 1)
DimPlot(seu, label = TRUE, repel = TRUE, cols = "polychrome") + NoAxes()
markers <- FindAllMarkers(seu, only.pos = TRUE)   # > c16 - unknown, c8 - immune
seu <- subset(seu, seurat_clusters %in% c(8,16), invert = TRUE)
...
DimPlot(seu, label = TRUE, repel = TRUE, cols = "polychrome") + NoAxes()
ggsave("cluster_res0.4.png", path = plot_path, width = 4, height = 3, units = "in", dpi = 150)
markers_res0.4 <- FindAllMarkers(seu, only.pos = TRUE)

# seu <- recluster(seu, npcs = 50, res = 2)
# DimPlot(seu, label = TRUE, repel = TRUE, cols = "polychrome") + NoAxes()
# ggsave("cluster_res2.png", path = plot_path, width = 4, height = 3, units = "in", dpi = 150)
# markers_res2 <- FindAllMarkers(seu, only.pos = TRUE)
...
features <- readLines(file.path("aux_data", "gene_set", "annotation", "fibroblast_markers.txt"))
sapply(features, save_fp, seu, fp_path)
...
# annotation ----
seu$celltype_2 <- ""
seu$celltype_2[seu$seurat_clusters %in% c(3)] <- "Fib1"
seu$celltype_2[seu$seurat_clusters %in% c(5,7)] <- "Fib2"
seu$celltype_2[seu$seurat_clusters %in% c(6)] <- "Fib3"
seu$celltype_2[seu$seurat_clusters %in% c(0)] <- "Fib-PC"
seu$celltype_2[seu$seurat_clusters %in% c(1)] <- "iCAF"
seu$celltype_2[seu$seurat_clusters %in% c(2)] <- "myoCAF"
seu$celltype_2[seu$seurat_clusters %in% c(4)] <- "matCAF"
seu$celltype_2[seu$seurat_clusters %in% c(8)] <- "prolifCAF"
seu$celltype_2 <- factor(seu$celltype_2, levels = c("Fib1", "Fib2", "Fib3", "Fib-PC", "iCAF", "myoCAF", "matCAF", "prolifCAF"))
...
# dotplot
features = c("Cd55", "Cd34", "Pdgfra", "Adamdec1", "Pcolce2", "Pi16", "Cym", "Col14a1", 
...
dotplot(kegg) +   theme(axis.text.y = element_text(size = 8, lineheight = 0.6),
                        axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
ggsave("kegg.png", path = plot_path, width = 6, height = 8, units = "in", dpi = 150)
...
dotplot(reactome) +   theme(axis.text.y = element_text(size = 8, lineheight = 0.6),
                            axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
ggsave("reactome.png", path = plot_path, width = 6, height = 8, units = "in", dpi = 150)
...
dotplot(wp) +   theme(axis.text.y = element_text(size = 8, lineheight = 0.6),
                      axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
ggsave("wp.png", path = plot_path, width = 6, height = 6, units = "in", dpi = 150)
...
ggsave("GSEA_H_hm.png", path = plot_path, width = 6, height = 8, units = "in", dpi = 150)
...
seu$celltype_2 <- factor(seu$celltype_2, levels = c(levels(seu$celltype)[-c(1)], levels(seu2$celltype_2)))
Idents(seu) <- "celltype_2"
