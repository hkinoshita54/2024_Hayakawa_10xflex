...
# set conditions ----
npcs = 15
path = paste0("plots/510_colon_lognorm_", "npc", as.character(npcs))
dir.create(path)
RDSfile = paste0("RDSfiles/seu_510_colon_lognorm_npc", as.character(npcs), ".RDS")


####################
# load data and filter by doublet calls, nFeature_RNA and percent.mt ----
seu <- readRDS(file = "RDSfiles/seu_02_filtered.RDS")
seu <- subset(seu, subset = exp_group == "colon")
...
####################
#### feature plots ####
files <- list.files(path = "gene_set/annotation_colon/", full.names = TRUE)
