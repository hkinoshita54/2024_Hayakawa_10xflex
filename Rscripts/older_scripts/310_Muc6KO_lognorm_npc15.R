...
# set conditions ----
npcs = 15
path = paste0("plots/310_Muc6KO_lognorm_", "npc", as.character(npcs))
dir.create(path)
RDSfile = paste0("RDSfiles/seu_310_T1PTC_lognorm_npc", as.character(npcs), ".RDS")


####################
# load data and filter by doublet calls, nFeature_RNA and percent.mt ----
seu <- readRDS(file = "RDSfiles/seu_02_filtered.RDS")
seu <- subset(seu, subset = orig.ident == "Muc6KO")
