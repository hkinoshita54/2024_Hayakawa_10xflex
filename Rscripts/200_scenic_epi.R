...
auc_mtx <- read_tsv(file = "pyscenic_docker/020_epi/auc_mtx.txt") %>% 
...
DimPlot(seu, group.by = "celltype2", label = TRUE, repel = TRUE, cols = pal$epi) + NoAxes()
...
Idents(seu) <- "celltype2"
saveRDS(seu, file = file.path("RDSfiles", "seu_200_scenic_epi.RDS"))

# edit rss.csv
rss <- read_csv("pyscenic_docker/020_epi//rss.csv")
...
# highlight <- "TEAD|SMAD|JUN|NFKB|RUNX"

rss_rankplot <- function(
    rss,
    celltype,
    out_dir
){
  rss2 <- rss %>%
    select(Regulon = Regulon, score = celltype) %>% 
    mutate(rank = rank(-score, ties.method = "average")) %>% 
    arrange(rank) %>% 
    slice_head(n = 50)
  
  rss3 <- rss2 %>%
    # filter(grepl(highlight, Regulon, ignore.case = TRUE))
    slice_head(n = 10)
  
  ggplot(rss2, aes(x = rank, y = score)) +
    ...
    labs(
      title = paste("Regulon Specific Score:", celltype),
      x = "Rank",
      y = "Score",
    ) +
  ...
  ggsave(
    paste0("rss_rank_", celltype, ".png"), 
    path = out_dir, width = 5, height = 5, dpi = 300)
}

rss_rankplot(rss = rss, celltype = "Epi-PTC", out_dir = plot_path)
rss_rankplot(rss = rss, celltype = "Epi-PC", out_dir = plot_path)
rss_rankplot(rss = rss, celltype = "Pit-PT", out_dir = plot_path)
